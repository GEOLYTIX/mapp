...
mapp.utils.versionCheck('4.13')
  ? console.log(`cluster_panel v4.13.0`)
  : console.warn(
      `Mapp version below v4.13.0. Please use the v4.8.0 cluster_panel plugin instead.`,
    );

mapp.utils.merge(mapp.dictionaries, {
  en: {
    cluster_panel: 'Icon Clustering',
...
mapp.plugins.cluster_panel = () => {
  console.warn(
    `The panel flag must be set in the layer.cluster config to add a cluster panel to the layer.view drawer.`,
  );
};
...
@param {Object} layer.cluster The layer default style.
@param {integer} [layer.cluster.distance] Distance for clustering in the browser [Openlayers].
@param {string} [layer.cluster.panel.distance_desc] Label for cluster distance slider.
@param {numeric} [layer.cluster.resolution] Resolution for clustering in the database.
@param {string} [layer.cluster.panel.resolution_desc] Label for cluster resolution slider.
@param {boolean} [layer.cluster.hexgrid] Enable hexgrid for resolution cluster.
@param {string} [layer.cluster.panel.hexgrid_desc] Label for hexgrid checkbox.
@returns {HTMLElement} A drawer element with controls for cluster parameter.
*/
function clusterPanel(layer) {
  ...
  if (layer.cluster.panel.icon_scale) {
    console.warn(
      `Layer ${layer.key} The layer.cluster_panel.icon_scale parameter is outdated. Please change this to layer.icon_scaling.clusterScale and use the icon_scaling plugin.`,
    );
  }
  ...
  if (layer.cluster.distance) {
    ...
    elements.push(
      mapp.ui.elements.slider({
        data_id: `${layer.key}-cluster-distance`,
        label:
          layer.cluster.panel.distance_desc ||
          mapp.dictionary.cluster_panel_distance,
        ...
        value: parseInt(layer.cluster.distance),
        callback: (e) => {
          // e.target.value deprecated in v4.10.0, so including e too
          layer.cluster.distance = parseInt(e?.target?.value || e);
  ...
  if (layer.cluster.resolution) {
    ...
    elements.push(
      mapp.ui.elements.slider({
        label:
          layer.cluster.panel.resolution_desc ||
          mapp.dictionary.cluster_panel_resolution,
        data_id: `${layer.key}-cluster-resolution`,
        min: 1,
        max: 100,
        // Val is Deprecated in v4.10.0 so including value too
        val: parseInt(1 / layer.cluster.resolution),
        value: parseInt(1 / layer.cluster.resolution),
        callback: (e) => {
          // e.target.value deprecated in v4.10.0, so including e too
          layer.params.resolution =
            (isFinite(1 / (e?.target?.value || e)) &&
              1 / (e?.target?.value || e)) ||
            0;
          clearTimeout(timeout);
          timeout = setTimeout(() => layer.reload(), 400);
  ...
  if (layer.cluster.hexgrid) {
    ...
    elements.push(
      mapp.ui.elements.chkbox({
        label:
          layer.cluster.panel.hexgrid_desc ||
          mapp.dictionary.cluster_panel_hexgrid,
        data_id: `${layer.key}-cluster-hexgrid`,
        checked: layer.cluster.hexgrid,
        onchange: (checked) => {
          layer.cluster.hexgrid = checked;
          layer.params.template = layer.cluster.hexgrid
            ? 'cluster_hex'
            : 'cluster';
          clearTimeout(timeout);
          timeout = setTimeout(() => layer.reload(), 400);
  ...
  const drawer = mapp.ui.elements.drawer({
    data_id: `${layer.key}-cluster-panel`,
    class: 'raised',
