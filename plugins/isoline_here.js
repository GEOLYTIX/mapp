...
The create a new location with a HERE Isoline geometry the isoline_here{} object must be added to the layer.draw configuration.
...
Without the "field" and "fieldfx" object entries isolines can still be requested from the HERE API and displayed in the mapview but the value with not be stored in the location data at rest.
...
if (parseInt(mapp.version.replace('.', '')) < 48) {
  ...
  console.warn(`isoline_here plugin requires mapp v4.8 or higher.`)
} else {

  console.log("isoline_here.js v4.8");
}
...
function isoline_here_draw_element(layer) {
  ...
  layer.draw.isoline_here.content = layer.draw.isoline_here.panel.map(el => mapp.ui.elements.isoline_here[el](layer.draw.isoline_here))
  ...
  layer.draw.isoline_here.btn = mapp.utils.html.node`
    <button
      ...
      onclick=${e => mapp.ui.elements.drawing.drawOnclick(e, layer, layer.draw.isoline_here)}>
  ...
  if (layer.draw.isoline_here.content.length) {
    ...
    return mapp.utils.html.node`<div>
      ...
      class: `drawer expandable ${layer.draw.isoline_here.groupClassList || ''}`,
      header,
      content
  ...
  if (entry.edit && entry.params.panel) {
    ...
    const editDrawer = mapp.ui.elements.drawer({
      ...
      content: mapp.utils.html.node`
        <div class="panel flex-col">
        ${entry.params.panel.map(el => mapp.ui.elements.isoline_here[el](entry.params))}
        <button
          class="flat wide bold primary-colour"
        onclick=${() => {
          if (entry.L) {
...
function datetime(params) {
  ...
  params.dateISO ??= params.datetime ? new Date(params.datetime).toISOString() : dateUTC;
  ...
  return mapp.utils.html`<div>
    ...
    <input
      ...
      let reverseDirectionChk = document?.querySelector('[data-id=reverse_direction] > input') || params.reverseDirection
      ...
      if (e.target.value) {
        ...
        if (reverseDirectionChk) {
          console.warn('reverse direction not valid in combination with a dateISO param, reverse direction disabled')
...
function shape(params) {
  ...
  if (!params.shapeMax || params.shapeMax < params.shape) {
    ...
    console.warn('A default params.shapeMax which bigger than param.shape is required for the UI element.')
  }
...
function rangeValues(params) {
  ...
  function arrInput(e) {
    ...
    params.rangeValues = e.target.value.split(',').map(val => parseInt(val) * 60)
  }
...
function decodePolylines(isolines, srid) {
  ...
  const coordinates = isolines.map(isoline => {
    ...
    const decoded = mapp.utils.here.decodeIsoline(isoline.polygons[0].outer, true)
    ...
    return decoded.polyline.map(coord => ol.proj.transform(coord, `EPSG:4326`, `EPSG:${srid}`))
  })
