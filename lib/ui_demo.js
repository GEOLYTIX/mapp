var Bt=Object.defineProperty;var Nt=(e,t)=>{for(var i in t)Bt(e,i,{get:t[i],enumerable:!0})};async function O(e){if(!(!e.dynamic&&e.create instanceof Function)){if(e.create instanceof Function){e.create();return}if(typeof e.target=="string"&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn("Dataviews require a HTMLHtmlElement target");return}return Pt(e)instanceof Error?e.err:(typeof e.label=="string"&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=Ht,e.show??=Mt,e.hide??=Vt,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}function Mt(){this.display=!0,this.create===void 0?(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create(),this.update instanceof Function&&this.update()):this.dynamic?(this.create(),this.update instanceof Function&&this.update()):this.reload&&this.update instanceof Function&&this.update(),this.btnRow?.style.setProperty("display","flex"),this.target.style.display="block"}function Vt(){this.display=!1,this.target.style.display="none",this.btnRow?.style.setProperty("display","none")}function Pt(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview="chartjs"),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns},e.dataview="tabulator"),e.table&&(e.dataview="tabulator")),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=new Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!="function")return e.err=new Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function Ht(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let i=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(i instanceof Error)){if(typeof this.responseFunction=="function"){this.responseFunction(i);return}typeof this.setData=="function"&&this.setData(i)}}function q(e){return e.title??=`${mapp.dictionary.information}`,e.data_id??="alert",e.class??="alert-confirm",e.header=mapp.utils.html`
    ...
    <div class="buttons">
      <button 
        ...
        onclick=${t=>{t.target.closest("dialog").close()}}>${mapp.dictionary.ok}`,mapp.ui.elements.dialog(e)}var Ge=e=>mapp.utils.html.node`
    <button 
        ...
        ${e.panel&&mapp.utils.html`
            <div class="panel">${e.panel}`}`;function F(e){return e.data_id??="card",mapp.utils.html.node`
  <div 
    data-id=${e.data_id}
    class="drawer">
    ...
    <span>${e.label}`}function I(e){return new Promise((t,i)=>{e.title??=`${mapp.dictionary.confirm}`,e.data_id??="confirm",e.header=mapp.utils.html`<h4>${e.title}`,delete e.minimizeBtn,delete e.closeBtn;let o=mapp.utils.html`<button 
      ...
      <div class="buttons">
        ${o}  
        ${l}`,e.class??="alert-confirm box-shadow",mapp.ui.elements.dialog(e)})}var Ze={draw:Ut,modify:Rt};function Rt(e){e?.preventDefault?.();let t=[];t.push(mapp.utils.html`
    <li
  ...
  <button
    ...
    style="display: grid; grid-template-columns: 100px 1fr; align-items: center;">
      ...
      <div style="grid-column: 2;">
        ${mapp.ui.elements.dropdown({callback:(l,n)=>Qe(i,n.option),entries:Object.values(i.unitsConfig),placeholder:i.unitsConfig[i.units].title})}`,i.rangeSlider=mapp.utils.html.node`<div>`,Qe(i,i.units),i.btn=mapp.utils.html.node`<button
    class="action wide"
    onclick=${l=>h(l,e,i)}>
    ...
    <h3>${mapp.dictionary.circle_config}</h3>`,content:o,popout:i.popout,dialog:i.dialog}),i.drawer)}function Qe(e,t){let i=e.unitsConfig[t];e.units=i.option,e.radius??=i.min,e.radius=e.radius>i.max?i.max:e.radius,mapp.utils.render(e.rangeSlider,mapp.ui.elements.slider({callback:o=>{e.radius=parseFloat(o)},max:i.max,min:i.min,val:e.radius}))}function li(e){return e.draw.locator={label:mapp.dictionary.draw_position,layer:e,type:"Point",...e.draw.locator},e.draw.locator.btn=mapp.utils.html.node`
    <button
      class="action wide"
      ...
      ${e.draw.locator.label}`,e.draw.locator.btn}function N(e){e.selectedTitles=new Set,e.selectedOptions=new Set,e.placeholder??=e.span||"",e.entries=e.entries?.filter?.(o=>o.option!==""),ai(e),si(e),e.onChange??=tt,e.options=ni(e);let t=e.selectedTitles.size?Array.from(e.selectedTitles).join(", "):e.placeholder,i=e.keepPlaceholder?e.placeholder:t;return e.placeHolderOption=mapp.utils.html.node`
  ...
  <svg
    ...
    <path
      ...
      stroke-dasharray=${e.lineDash} />`,o=`data:image/svg+xml,${encodeURIComponent(ot.serializeToString(i))}`,l=`
    background-position: center;
    background-repeat: no-repeat;
    background-size: contain;
    width: ${e.width}px;
  ...
  <svg
    ...
    <a 
      class="switch"
      href=${e.register}>${e.login_registration_link}</a>`;function s(){n.disabled=!(i.validity.valid&&l.validity.valid),o.textContent=n.disabled?e.login_invalid:""}}function J(e){e.placeholder??="",e.data_id??="numeric-input",e.numericChecks??=ki;let t=mapp.utils.formatNumericValue(e),o=`text-align: center; width: ${e.dynamicWidth?t.length+1.3+"ch":"100%"}`;return mapp.utils.html.node`<input
    data-id=${e.data_id}
    type="text"
    style=${o}
    ...
    oninput=${n=>nt(n,e)}>`}function nt(e,t){if(t.stringValue=e.target.value,t.numericChecks(e.target.value,t)?(t.newValue=t.onRangeInput?t.stringValue:mapp.utils.unformatStringValue(t),delete t.invalid,e.target.classList.remove("invalid"),t.sliderElement&&(t.sliderElement.style.setProperty(`--${e.target.dataset.id}`,t.newValue),t.sliderElement.querySelector(`[name=${t.rangeInput}]`).value=t.newValue)):(t.invalid=!0,e.target.classList.add("invalid")),t.invalid)return t.callback();t.callback(t.newValue),t.dynamicWidth&&(e.target.style.width=e.target.value.length+1.3+"ch"),t.onRangeInput=!1}function ki(e,t){return t.onRangeInput&&e===null||isNaN(e)||t.min&&e<t.min?!1:t.max?e<=t.max:!0}function G(e={}){return e.container=mapp.utils.html.node`<div class="pill-container">`,e.pills=Array.isArray(e.pills)?new Set(e.pills):new Set,e.add=yi,e.remove=Li,e.pills.forEach(t=>e.add(t)),e.target instanceof HTMLElement&&e.target.append(e.container),e}function yi(e){let t=this,i=mapp.utils.html.node`<div
    class="pill"
    style=${t.css_pill}
    data-value=${e}
  ...
  <input
    ...
    <div class="searchbox">
      ${e.input}
      ${e.list}`,e.target.append(e.node),e.input.addEventListener("input",t=>e.searchFunction(t)),e.input.addEventListener("focus",t=>e.searchFunction(t)),e}function K(e){e.group_id??=e.data_id||"slider",e.data_id="a",e.rangeInput="rangeInput",e.step??=1,e.value??=e.val,e.title??="",e.style??="";let t=mapp.ui.elements.numericInput(e);return e.sliderElement=mapp.utils.html.node`
    <div
      role="group"
      data-id=${e.group_id}
      title=${e.title}
    ...
    <div
      ...
      <input data-id="b" type="range"
        ...
        step=${e.step}
        value=${e.val_b}
        oninput=${p=>u(p,e)}/>`;t.sliderElement=c,o.sliderElement=c;function u(p,w){let Je=Number(p.target.value);p.target.dataset.id==="a"&&(t.onRangeInput=!0,i.value=Je,i.dispatchEvent(new Event("change"))),p.target.dataset.id==="b"&&(o.onRangeInput=!0,l.value=Je,l.dispatchEvent(new Event("change")))}return e.max-e.min?c:mapp.utils.html.node`<div>No available range for slider element.`}function te(){return mapp.utils.html`<div
  class="switch-all"
  style="grid-column: 1/3;">
  ${mapp.dictionary.layer_style_switch_caption}
  <button
    class="bold"
    onclick=${e=>{let t=[...e.target.closest(".legend").querySelectorAll(".switch")],i=t.filter(o=>o.classList.contains("disabled"));i.length==0||i.length==t.length?t.forEach(o=>o.click()):i.forEach(o=>o.click())}}>${mapp.dictionary.layer_style_switch_all}</button>.`}function ie(e={}){return e.data_id??="ui-elements-toast",e.content??="Set custom HTML content in element configuration.",e.actions??=[],e.accept&&e.actions.push({label:typeof e.accept=="string"?e.accept:"Accept"}),e.cancel&&e.actions.push({label:typeof e.accept=="string"?e.cancel:"Cancel"}),new Promise(t=>{let i;e.close&&(i=mapp.utils.html`
      ...
      <div class="actions">
      ${o}
      </div>`;document.body.append(n);function a(s,d){n.classList.add("ease-out"),setTimeout(function(){n.remove()},1200),d?.callback instanceof Function&&d.callback(s),t(s?.target.value)}})}function oe(e={}){if(e.data_id??="ui-elements-tooltip",!e.content){console.warn(`mapp.ui.elements.tooltip: content for ${e.data_id} not provided.`);return}return e.node=mapp.utils.html.node`<span 
    data-id=${e.data_id}
    title=${e.content}
    class="tooltip material-symbols-outlined notranslate">help</span>`,e.node}function le(e,t){return e.localeInput=mapp.utils.html.node`<input
  ...
  <div>
    ...
    ${e.ulLocales}`,ne(e,t),e.panel}async function Ei(e,t,i){e.panel.classList.add("disabled"),t.locale.name=i,await mapp.utils.userLocale.remove(t.locale),ne(e,t),e.panel.classList.remove("disabled")}async function Si(e,t){e.panel.classList.add("disabled"),t.locale.name=encodeURIComponent(e.localeInput.value),await mapp.utils.userLocale.putLocale(t)===t.locale.name&&(ne(e,t),e.panel.classList.remove("disabled"))}async function ne(e,t){let i=await mapp.utils.userLocale.list(t.locale.workspace);if(!i.length){mapp.utils.render(e.ulLocales,mapp.utils.html`<div>`);return}let o=i.map(l=>{let n=mapp.utils.html`<button
    ...
    <ul>${o}`)}var at={alert:q,btnPanel:Ge,card:F,chkbox:z,confirm:I,contextMenu:Ze,control:D,dialog:A,drawer:Xe,drawerDialog:B,drawing:Ke,dropdown:N,helpDialog:V,jsoneditor:P,layerStyle:it,legendIcon:R,locales:U,loginForm:W,numericInput:J,pills:G,radio:Z,reportLink:X,registerForm:Y,searchbox:Q,slider:K,slider_ab:ee,themeLegendSwitch:te,toast:ie,tooltip:oe,userLocale:le};var st=e=>{e={...e,...mapp.ui.elements.searchbox({name:"gazetteer-search-input",placeholder:e.placeholder,searchFunction:t,target:e.target})};function t(i){if(e.list.innerHTML="",!i.target.value.length)return;let o=i.target.value.split(",").map(parseFloat);if(o.length===2&&o.every(l=>typeof l=="number"&&!isNaN(l)&&isFinite(l))){let[l,n]=o;if(l>=-90&&l<=90&&n>=-180&&n<=180){e.list.appendChild(mapp.utils.html.node`
  ...
  <input
    ...
    <div style="
      ...
      <label>Date after
        ${i}</label>
      <label>Date before
        ${o}</label>`}function ae(e){let t=e.style.theme;t.legend??={},t.legend.grid=[],t.legend.alignContents??="left",t.legend.alignContents+=" contents",e.style.theme.style??={},e.style.theme.style.width??=24,e.style.theme.style.height??=24;let i=mapp.ui.elements.legendIcon(e.style.theme.style);return t.legend.grid.push(mapp.utils.html`
    <div 
      class="contents">
      ${i}<div class="label" style="grid-column: 2";>${e.style.theme.label}`),t.legend.node=mapp.utils.html.node`
    <div class="legend">
    <div class="contents-wrapper grid">${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}function $(e,t,i){if(e.field??=t.field,e.disabled=i.filter?.current[e.field]?.ni?.includes(e.value),i.featureFields&&t.distribution==="count"){let d={value:i.featureFields[e.field]?.[e.value]};if(e.count=mapp.utils.formatNumericValue(d),!e.disabled&&!e.count)return}let o=mapp.ui.elements.legendIcon({height:24,width:24,...e._style||e.style}),l=mapp.utils.html`<div
    ...
    class=${n}>
    ${a}`;t.legend.switch&&(s.onclick=d=>Ai(d,i,e)),e.node=mapp.utils.html.node`<div
    data-id=${e.value}
    class="${t.legend.alignContents}">
    ${l}${s}`,t.legend.grid.push(e.node)}async function Ii(e,t){if(e.style.theme.filterOnly)return;if(Array.isArray(t.keys))for(let o of o.keys)e.filter.current[o.field].ni.splice(e.filter.current[o.field].ni.indexOf(key),1);else e.filter.current[t.field].ni.splice(e.filter.current[t.field].ni.indexOf(t.value),1);e.filter.current[t.field].ni.length||(delete e.filter.current[t.field].ni,Object.keys(e.filter.current[t.field]).length||delete e.filter.current[t.field]);let i=e.filter.list?.find(o=>o.type==="ni"&&o.field===t.field);if(i?.card){let o=await mapp.ui.layers.filters.ni(e,i);i.card.querySelector(".filter").replaceWith(o)}}async function Di(e,t){if(e.style.theme.filterOnly)return;e.filter.current[t.field]||(e.filter.current[t.field]={}),e.filter.current[t.field].ni||(e.filter.current[t.field].ni=[]),e.filter.current[t.field].ni.push(t.keys||t.value),e.filter.current[t.field].ni=e.filter.current[t.field].ni.flat();let i=e.filter.list?.find(o=>o.type==="ni"&&o.field===t.field);if(i?.card){let o=await mapp.ui.layers.filters.ni(e,i);i.card.querySelector(".filter").replaceWith(o)}}function Ai(e,t,i){e.target.classList.toggle("disabled")?(i.disabled=!0,Di(t,i),i._style=i.style,i.style=null):(delete i.disabled,Ii(t,i),i._style??=i.style,i.style=i._style,delete i._style),t.style.theme.filterOnly?t.L.changed():t.reload()}function _(e){let t=e.display?"block":"none",i=e.style.theme;return i.legend.node.style.setProperty("display",t),e.style.legend?.style.setProperty("display",t),i.meta_node?.style.setProperty("display",t),i.legend.node}function x(e){let t=e.style.theme,i=mapp.utils.html`<div
    style="height: 40px; width: 40px;">
    ${mapp.ui.elements.legendIcon({height:40,icon:e.style.cluster.icon,width:40})}`,o=mapp.utils.html`<div
    class="label">
    ${mapp.dictionary.layer_style_cluster}`;t.legend.grid.push(mapp.utils.html`<div
    data-id="cluster"
    class=${t.legend.alignContents}>
    ${i}${o}`)}function k(e){let t=e.style.theme;t.filterOnly??=e.style.filterOnly,t.legend??={},t.legend.grid=[],t.legend.alignContents??="left",t.legend.alignContents.includes("contents")||(t.legend.alignContents+=" contents"),t.legend.classList=`contents-wrapper ${t.legend?.layout||"grid"} ${t.legend?.nowrap?"nowrap":""}`,t.legend.style=t.legend?.nowrap?"overflow: scroll;":""}function se(e){k(e);let t=e.style.theme;return t.legend.switch=e.filter&&mapp.ui.elements.themeLegendSwitch(),t.categories.forEach(i=>{$(i,t,e)}),e.style.cluster&&x(e),t.legend.node=mapp.utils.html.node`
    <div class="legend">
      ${t.legend.switch||""}
      <div class=${t.legend.classList} style=${t.legend.style}>
        ${t.legend.grid}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.distribution==="count"?(_(e),t.legend.node):(e.style.legend.style.removeProperty("display"),t.legend.node)}function y(e){k(e);let t=e.style.theme;if(!t.hideLegend){for(let i of t.categories)i.label=i.values?.join(", "),i.label!==void 0&&$(i,t,e);return e.style.cluster&&x(e),t.legend.node=mapp.utils.html.node`
    <div class="legend">
      ${t.legend.switch||""}
      <div class=${t.legend.classList} style=${t.legend.style}>
        ${t.legend.grid}`,_(e),e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),e.L.once("postrender",()=>{t.key===e.style.theme.key&&y(e)}),t.legend.node}}function de(e){let t=e.style.theme;t.legend??={};let i=mapp.ui.elements.themeLegendSwitch(),o=t.categories.filter(a=>a.value!==void 0).map(a=>{let s=`contents ${t.legend?.horizontal?"horizontal":""}`,d=mapp.ui.elements.legendIcon({height:24,width:24,...a._style||a.style});a.label??=a.value;let c=`label switch ${a.disabled?"disabled":""}`;return mapp.utils.html`<div 
        data-id=${a.value}
        class=${s}>
        <div 
          style="height: 24px; width: 24px; grid-column: 1;">
    ...
    <div class="legend">
      ...
      <div class=${l} style=${n}>
        ${o}
      </div>`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}function Bi(e,t,i){e.target.classList.toggle("disabled")?(i.disabled=!0,i._style=i.style,i.style=null):(delete i.disabled,i.style=i._style,delete i._style),t.L.changed()}var gt={basic:ae,categorized:se,distributed:y,graduated:de};function ce(e){if(!e.target)return;let t={add:Ni,createGroup:Mi,groups:{},node:e.target};return typeof e.layers=="object"&&Object.values(e.layers).length&&Object.values(e.layers).forEach(i=>t.add(i)),t}function Ni(e){if(!e.hidden){if(mapp.ui.layers.view(e),!e.group){this.node.append(e.view),this.node.dispatchEvent(new CustomEvent("addLayerView",{detail:e}));return}this.groups[e.group]||this.createGroup(e),this.groups[e.group].addLayer(e),this.node.dispatchEvent(new CustomEvent("addLayerView",{detail:e}))}}function Mi(e){let t={list:[]};this.groups[e.group]=t,t.hideLayers=mapp.utils.html.node`<button
  class="notranslate material-symbols-outlined active"
    ...
    onclick=${i=>{i.target.style.visibility="hidden",t.list.filter(o=>o.display).forEach(o=>o.hide())}}>visibility_off`,t.meta=mapp.utils.html.node`<div class="meta no-select">`,t.drawer=mapp.ui.elements.drawer({class:`layer-group ${e.groupClassList||""}`,content:t.meta,data_id:e.group,header:mapp.utils.html`
       <h2>${e.group}</h2>
       ${t.hideLayers}`}),this.node.appendChild(t.drawer),t.chkVisibleLayer=Pi,t.addLayer=Vi}function Vi(e){if(e.groupmeta){let t=mapp.utils.html.node`<div>`;t.innerHTML=e.groupmeta,mapp.utils.render(this.meta,t)}this.list.push(e),this.drawer.appendChild(e.view),this.chkVisibleLayer(),e.showCallbacks.push(()=>this.chkVisibleLayer()),e.hideCallbacks.push(()=>this.chkVisibleLayer())}function Pi(){this.hideLayers.style.visibility=this.list.some(e=>e.display)?"visible":"hidden"}function re(e){let t=[];for(let[o,l]of Object.entries(e.dataviews))if(!(typeof l=="string"||l===!0||l===!1)){if(Object.assign(l,{host:e.mapview.host,key:o,layer:e}),Hi(l),l.tabview&&mapp.utils.mobile()||(l.label??=l.title||l.key,mapp.ui.utils.dataviewDialog(l),l.target=l.target instanceof HTMLElement?l.target:mapp.utils.html.node`
    ...
    data-id=resetall
    class="flat underline"
    onclick=${t=>{e.filter.list.forEach(i=>mapp.ui.layers.filters.resetFilter(e,i))}}>${mapp.dictionary.filter_reset_all}`,e.filter.count=mapp.utils.html.node`<span class="bold">`,e.changeEndCallbacks.push(pe),e.hideCallbacks.push(t=>{t.filter.clearAll?.checkVisibility()&&t.filter.feature_count.style.setProperty("display","none")}),e.filter.count_meta??=mapp.dictionary.filter_count,e.filter.feature_count=mapp.utils.html.node`
    ...
    <div class="link-with-img">
      ...
      data-src=${e.value}
      onclick=${i=>$e(i,e,mapp.dictionary.remove_image_confirm)}>delete`;return mapp.utils.html.node`<div class="img-item"><img
      style="width: 100%"
      src=${e.value}
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{v(t,e)}}>`}function to(e){let t=[];if(e.value?.map(i=>{let o=e.edit&&mapp.utils.html`<button title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${i.replace(/^.*\//,"").replace(/\.([\w-]{3})/,"")}
      data-src=${i}
      onclick=${l=>$e(l,e,mapp.dictionary.remove_image_confirm)}>delete`;t.push(mapp.utils.html`<div class="img-item"><img 
      src=${i}
      onclick=${mapp.ui.utils.imagePreview}>
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${o=>{v(o,e)}}>`;t.push(i)}if(t.length)return mapp.utils.html.node`<div class="images-grid">${t}`}function io(e){let t=[];if(e.value?.map(i=>{let o=e.edit&&mapp.utils.html`<button 
      title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${i.replace(/^.*\//,"").replace(/\.([\w-]{3})/,"")}
      data-href=${i}
      onclick=${n=>$e(n,e,mapp.dictionary.remove_document_confirm)}>delete`,l=i.replace(/^.*\//,"").replace(/\.([\w-]{3})/,"");t.push(mapp.utils.html`<div class="link-with-img">
      ...
      <input type="file"
        ...
        onchange=${o=>v(o,e)}>`;t.unshift(i)}if(t.length)return mapp.utils.html.node`<div>${t}`}async function v(e,t){let i=new FileReader,o=e.type==="drop"?e.dataTransfer?.files[0]:e.target.files[0];if(!o)return;t.location.view?.classList.add("disabled");let l={documents:lo,image:xt,images:xt};i.onload=n=>l[t.type](n,t,o),i.readAsDataURL(o)}function xt(e,t,i){let o=new Image;o.onload=async()=>oo(t,o,i),o.src=e.target.result}async function oo(e,t,i){let o=document.createElement("canvas");e.max_size??=1024,t.width>t.height&&t.width>e.max_size?(t.height*=e.max_size/t.width,t.width=e.max_size):t.height>e.max_size&&(t.width*=e.max_size/t.height,t.height=e.max_size),o.width=t.width,o.height=t.height,o.getContext("2d").drawImage(t,0,0,t.width,t.height);let l=i.name.replace(/^.*\//,"").replace(/\.([\w-]{3})/,"")+e.suffix_date?`@${Date.now()}`:"",n=await xe(e,{public_id:l});if(!n)return;let a=new FormData;a.append("file",o.toDataURL("image/jpeg",.5));let s=await fetch(n,{body:a,method:"post"});if(!s||s.error){let u=`Cloudinary Image upload failed! ${s?.error?.message?`Error: ${s.error.message}`:""}`;mapp.ui.elements.alert({text:u});return}let d=await s.json();e.type==="image"?e.value=d.secure_url:e.value=Array.isArray(e.value)?e.value.concat([d.secure_url]):[d.secure_url],_e(e)}async function lo(e,t,i){let o=new Date,l=`${o.getMonth()+1}-${o.getDate()}T${o.getHours()}:${o.getMinutes()}`,n=i.name.substring(i.name.lastIndexOf(".")),a=`${i.name.replace(n,"")}-${l}${n}`,s=await xe(t,{public_id:a});if(!s)return;let d=new FormData;d.append("file",e.target.result.toString());let c=await fetch(s,{body:d,method:"post"});if(!c||c.error){let b=c?.error?.message?`Error: ${c.error.message}`:"";mapp.ui.elements.alert(`Cloudinary document upload failed! ${b}`);return}let u=await c.json();t.value=Array.isArray(t.value)?t.value.concat([u.secure_url]):[u.secure_url],_e(t)}async function $e(e,t,i){if(!await mapp.ui.elements.confirm({text:i}))return;let l=decodeURIComponent(e.target.dataset.name),n=await xe(t,{destroy:!0,public_id:l});if(!n)return;await fetch(n,{method:"post"});let a=new Set(t.value);a.delete(e.target.dataset.src||e.target.dataset.href),t.type==="image"?t.value=null:t.value=a.size?Array.from(a):null,_e(t)}async function _e(e){e.location.view?.classList.add("disabled"),await mapp.utils.xhr({body:JSON.stringify({[e.field]:e.value}),method:"POST",url:`${e.location.layer.mapview.host}/api/query?`+mapp.utils.paramString({id:e.location.id,layer:e.location.layer.key,locale:e.location.layer.mapview.locale.key,table:e.location.table,template:e.location.updateTemplate})}),e.node.replaceChildren(mapp.utils.html.node`<div class="label">${e.title}`,kt[e.type](e)),e.location.view?.classList.remove("disabled")}async function xe(e,t){let i=mapp.utils.paramString({...t,folder:e.cloudinary_folder}),o=await mapp.utils.xhr({responseType:"text",url:`${e.location.layer.mapview.host}/api/sign/cloudinary?${i}`});return o instanceof Error&&console.error(o),o}function ke(e){if(e.value!==void 0&&(e.data=e.value),e.data?.length===0&&(e.data=null),e.layer??=e.location.layer,no(e),e.target instanceof HTMLElement&&!(mapp.ui.Dataview(e)instanceof Error))return(!e.data||e.data instanceof Error)&&e.queryCheck&&(e.chkbox?.classList?.add?.("disabled"),e.chkbox.querySelector("input").checked=!1,e.display=!1,e.hide()),e.display&&e.show?.(),mapp.utils.html.node`
    ${e.chkbox||""}
  ...
  <div
    class="val"
    style="${e.css_val}">${t}`;return!e.edit&&e.type==="html"&&(i.innerHTML=e.value||""),i}function qe(e){let t,i=e.value?.toString().replace(".",":");return i=i&&i.length<3&&`${i}:00`||i,e.edit?t=mapp.utils.html.node`
      <input
        type="time"
        value=${i}
  ...
  <div
    ...
    class=${t}
    onclick=${i=>{let o=i.target.classList.toggle("active");e.viewport=o,e.update()}}>${mapp.dictionary.tabulator_viewport}`}function Me(e){if(e.target!=="dialog")return;e.dataview_dialog={};let t=`
    ...
    overflow: hidden !important`;Object.assign(e.dataview_dialog,{header:mapp.utils.html.node`<h1> ${e.label}`,data_id:`${e.key}-dataviews-dialog`,target:document.getElementById("Map"),height:"auto",left:"5%",top:"0.5em",class:"box-shadow tabview",css_style:t,containedCentre:!0,contained:!0,headerDrag:!0,closeBtn:!0,onClose:()=>{e.chkbox&&(e.chkbox.querySelector("input").checked=!1)}}),mapp.ui.elements.dialog(e.dataview_dialog);let i=e.dataview_dialog.node.querySelector(".content");i.classList.add("panel"),e.target=i,e.dataview_dialog.dialog.close(),e.show=()=>{e.update(),e.toolbar&&e.dataview_dialog.node.querySelector(".btn-row").style.removeProperty("display"),e.chart&&(e.dataview_dialog.node.appendChild(e.target.querySelector("canvas")),e.dataview_dialog.node.removeChild(e.target)),e.dataview_dialog.dialog.show()},e.hide=()=>{e.dataview_dialog.dialog.close()},e.location?.removeCallbacks?.push?.(()=>e.dataview_dialog.dialog.close())}var tl={create:il},qt=tl;async function il(e){e.query??="histogram",e.options??={};let t=`column-gap: ${e.options.columnGap||10}px`;e.node=mapp.utils.html.node`<div class="histogram" style=${t}>`,e.options.width&&(e.node.style.width=`${e.options.width}px`),e.options.height&&(e.node.style.height=`${e.options.height}px`),e.options.title&&e.target.append(mapp.utils.html.node`<h3 class="histogram-title">${e.options.title}`),e.setData=ll,e.target.append(e.node),e.options.xlabel&&(e.xlabel=mapp.utils.html.node`<div class="histogram-xlabel">${e.options.xlabel}`,e.options.width&&(e.xlabel.style.width=`${e.options.width}px`),e.target.append(e.xlabel)),e.options.caption&&(e.caption=mapp.utils.html.node`<span class="histogram-caption">${e.options.caption}`,e.target.append(e.caption))}function ll(e){this.data=e,nl(this)}function nl(e){if(!Array.isArray(e.data)){mapp.utils.render(e.node,mapp.utils.html`<div>`);return}let t=Math.max(...e.data.map(l=>l.count));e.options.height??=100;let i=e.data.map(l=>{let n=`height: ${Math.round(e.options.height*l.count/t)}px`,a="";return e.options.tooltip&&(a=mapp.utils.html`<div class="bucket-info">
      ...
      style=${n}>
      ${a}`}),o=e.options.ylabel?mapp.utils.html`<div class="ylabel">${e.options.ylabel}`:"";mapp.utils.render(e.node,mapp.utils.html`${o}${i}`)}var m={idle:600};function Ve(e){Object.assign(m,e),m.idle!==0&&(window.onload=g,window.onmousemove=g,window.onmousedown=g,window.ontouchstart=g,window.onclick=g,window.onkeypress=g,g(),zt())}function g(){m.locked||(m.timeout&&clearTimeout(m.timeout),m.timeout=setTimeout(Ft,m.idle*1e3))}function Ft(){m.locked=!0,m.renew&&clearTimeout(m.renew);let e={url:`${m.host}/api/user/cookie?destroy=true`};e.onLoad=t=>location.reload(),mapp.utils.xhr(e)}function zt(){m.renew=setTimeout(al,(m.idle-20)*1e3)}function al(){let e={url:`${m.host}/api/user/cookie?renew=true`};e.onLoad=t=>{if(t.target.status===401)return Ft();zt()},mapp.utils.xhr(e)}function Pe(e){let t=mapp.utils.html.node`
...
`)}`});return}e.setData(i)}}>`,e.toolbar.csvupload.input}function pl(e){return e.props={mainMenuBar:!1,mode:"text"},mapp.utils.html.node`<button onclick=${()=>{let i=e.jsoneditor.get();if(i.text==="")return;let o=mapp.utils.paramString(e.toolbar.sqlinsert),l=`${mapp.host}/api/query?${o}`;return mapp.utils.xhr({body:i.text,method:"POST",url:l})}}>SQL Insert</button>`}async function He(e,t,i){return await mapp.ui.elements.jsoneditor({props:{onRenderMenu:l=>ml(l,i),content:t,mode:"text"},target:e})}function ml(e,t){return e.push(t),e.filter(i=>i.text!=="text").filter(i=>i.text!=="tree").filter(i=>i.text!=="table").filter(i=>i.type!=="separator").filter(i=>i.className!=="jse-undo").filter(i=>i.className!=="jse-redo").filter(i=>i.className!=="jse-search").filter(i=>i.className!=="jse-contextmenu").filter(i=>i.className!=="jse-sort").filter(i=>i.className!=="jse-transform")}async function Re(e){let t={layer:e,queryparams:{}};t.viewport=e.filter?.viewport||e.queryparams?.viewport;let i=mapp.utils.queryParams(t),o=mapp.utils.paramString({...i,filter:e.filter?.current,layer:e.key,table:e.tableCurrent(),qID:e.qID,template:"location_count"});return await mapp.utils.xhr(`${e.mapview.host}/api/query?${o}`)}var Ue={},r={X:{min:75,lockSize:200,grid:"gridTemplateColumns"},Y:{min:0,lockSize:50,grid:"gridTemplateRows"},gridTemplate:{X:"auto",Z:"10px",Y:"auto"}};function We(e){e.resizeEvent??=fl.bind(e),e.stopResize=gl.bind(e),e.axis??="X",e.doubleClickClose&&(e.target.ondblclick=t=>hl(t,e)),e.target.addEventListener("mousedown",t=>{t.preventDefault(),document.body.style.cursor="grabbing",globalThis.addEventListener("mousemove",e.resizeEvent),globalThis.addEventListener("mouseup",e.stopResize)}),e.target.addEventListener("touchstart",t=>{t.preventDefault(),globalThis.addEventListener("touchmove",e.resizeEvent),globalThis.addEventListener("touchend",e.stopResize)},{passive:!0}),Ue.gridTemplateColumns=getComputedStyle(document.body).gridTemplateColumns,Ue.gridTemplateRows=getComputedStyle(document.body).gridTemplateRows}function fl(e){let t=this.axis;r.X.size=e.touches?.[0]?.pageX||e.pageX,r.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY);let i=r[t];if(i.size<i.min){r.gridTemplate[t]="0px",document.body.style[i.grid]=Object.values(r.gridTemplate).join(" ");return}i.size<i.lockSize||(t==="X"&&r.X.size>globalThis.innerWidth/2&&(r.X.size=globalThis.innerWidth/2),t==="Y"&&(r.Y.size>globalThis.innerHeight-10&&(r.Y.size=globalThis.innerHeight),OL.style.marginTop=`-${r.Y.size/2}px`),r.gridTemplate[t]=`${i.size}px`,document.body.style[i.grid]=Object.values(r.gridTemplate).join(" "),r.gridTemplate={X:"auto",Z:"10px",Y:"auto"})}function hl(e,t){if(r.X.size=e.touches?.[0]?.pageX||e.pageX,r.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY),r[t.axis].size<r[t.axis].min){document.body.style[r[t.axis].grid]=Ue[r[t.axis].grid];return}r.gridTemplate[t.axis]="0px",document.body.style[r[t.axis].grid]=Object.values(r.gridTemplate).join(" "),r.gridTemplate={X:"auto",Z:"10px",Y:"auto"}}function gl(){document.body.style.cursor="auto",globalThis.removeEventListener("mousemove",this.resizeEvent),globalThis.removeEventListener("touchmove",this.resizeEvent),globalThis.removeEventListener("mouseup",this.stopResize),globalThis.removeEventListener("touchend",this.stopResize)}var Dt={cssColour:Ne,dataview:Ot,dataviewDialog:Me,histogram:qt,idleLogout:Ve,imagePreview:Pe,Json:It,layerJSE:He,locationCount:Re,resizeHandler:We};var At={Dataview:O,elements:at,Gazetteer:st,layers:_t,locations:Tt,Tabview:Ae,utils:Dt};globalThis.ui=At;globalThis.mapp??={};mapp.ui=At;
